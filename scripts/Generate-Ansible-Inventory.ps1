...
'' | Out-File -Force -FilePath .\inventory -Encoding utf8

$activeVMs = Get-VM | Where-Object { !$excludeVMs.Contains($_.Name) }

$activeVMs | ForEach-Object {
    "$($_.Name) ansible_ssh_host=$($_.NetworkAdapters[0].IPAddresses[0])" | Out-File -Append -FilePath .\inventory -Encoding utf8
}

"`n[datacenter]" | Out-File -Append -FilePath .\inventory -Encoding utf8
$activeVMs.Name | Out-File -Append -FilePath .\inventory -Encoding utf8

$groups.Keys | ForEach-Object {
    $machines = $groups.$_ | Where-Object { $activeVMs.Name.Contains($_) }
    if ($machines.Count -gt 0)
    {
        "`n[$_]" | Out-File -Append -FilePath .\inventory -Encoding utf8
        $machines | Out-File -Append -FilePath .\inventory -Encoding utf8
    }
}
